...
function Cart(){
    ...
    return(
        <Container className="py-5 my-1">
            <h1 className="main-f text-center">Welcome To Cart</h1>
            <button className="btn btn-outline-danger mb-2 float-end w-25" onClick={() => dispatch(clear())}>Clear</button>
            <h5 className="name-f">Total Price: {totalPrice.toFixed(2)} $</h5>
            <Table striped bordered hover size="sm" className="name-f">
                <thead>
                    <tr>
                    <th>#</th>
                    <th>Title</th>
